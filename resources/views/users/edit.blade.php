        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    @auth
                    @if ($isadmin)
                    <form class="flex flex-col gap-4" action="{{ route('users.update', $id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <select name="pos" id="pos" class="thor-input-field">
                            @foreach ($positions as $position)
                                <option value="{{$position->id}}" {{ $position->id == old('pos', $pos) ? 'selected="selected"' : ''}}>{{$position->name}}</option>
                            @endforeach
                        </select>
                        @error('pos')
                        ...
                        <input type="text" name="email" id="email" class="thor-input-field" value="{{ old('email', $email) }}">
                        @error('email')
                        ...
                        <input type="text" name="phone" id="phone" class="thor-input-field" value="{{ old('phone', $phone) }}">
                        @error('phone')
                        ...
                        <input type="text" name="card" id="card" class="thor-input-field" value="{{ old('card', $card) }}">
                        @error('card')
